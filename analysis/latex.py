...
def generate_datasets_table(datasets, filters_var_type, PAPER_LOCATION):

    for vartype, vartypefunction in filters_var_type:
        fname = os.path.join(PAPER_LOCATION, "datasets", "generated_table_datasets_{}.tex".format(vartype))
        ensure_folder_exists(os.path.join(PAPER_LOCATION, "datasets"))
