...
from predictions.performance_comparison import generate_performance_comparison, perform_intron_analysis
...
def run_standard_analysis(dataset, all_data, out_dir, clinvarStars, intronic_analysis, thresholdAnalysis,
                          scope_to_predict,
                          types_to_analyse, machineLearning, skipHeatmap, new_thresholds_done):
    if dataset == "clinvar":
        df_to_evaluate = all_data[clinvarStars]
        out_dir = os.path.join(out_dir, dataset, clinvarStars)
    else:
        dataset = utils.check_dataset_arg(dataset)
        df_to_evaluate = all_data[dataset]
        out_dir = os.path.join(out_dir, dataset)

    generate_datasets_table(all_data, filters_var_type, out_dir)
    threshold_list = subset_toolset_by_scope(threshold_list_complete, scope_to_predict)
    variant_types_to_evaluate = subset_variants_by_type(filters_var_type, types_to_analyse)
    absent_tools = generate_performance_comparison(df_to_evaluate, variant_types_to_evaluate, filters, threshold_list,
                                                   dataset, out_dir)
    ...
    if intronic_analysis:
        perform_intron_analysis(df_to_evaluate, filter_intronic_bins, threshold_list, dataset, out_dir)

    if skipHeatmap is False:
        generate_heatmap(df, filters_var_type, filters, threshold_list, dataset, out_dir, absent_tools)
    ...
    if thresholdAnalysis and new_thresholds_done is False:
        ...
        generate_performance_comparison(df_to_evaluate, filters_var_type, filters, threshold_list, dataset, out_dir,
...
def main():
    """ Tool to analyse variant prediction methods """
  ...
  #                                                                                 "specific analysis of Clinvar variants.It does not depend on the --limitAnalysis argument.")
    ...
    if unseen_vcf_analysis:
        ...
        inspect_predictions(args.vcf_file, args.top_tools, args.n_top_tools, args.plot_tool, args.s,
                            variant_types_to_evaluate, args.labels, args.location, args.intronic, OUT_DIR)
        ...
        if args.datasets:
            ...
            for dataset in args.datasets:
                ...
                if dataset in possible_datasets:
                    all_data = preprocess(args.location, ROOT_DIR, args.thresholdAnalysis, args.clinvarStars,
                                          args.intronic,
                                          dataset=dataset)
                else:
                    all_data = preprocess(args.location, ROOT_DIR, args.thresholdAnalysis, args.clinvarStars,
